...
- name: Install Java on Pathnex server
  ...
  tasks:
    ...
    - name: Set JAVA_HOME
      lineinfile:
        path: /etc/profile
        line: 'export JAVA_HOME=/user/lib/jvm/java-11-openjdk'
...
build:
  ...
  script:
    ...
    - cd sample-java-app
    - mvn clean package
    - curl -X POST -H 'Content-type: application/json' --data '{"text":"Build success for Pathnex"}'
